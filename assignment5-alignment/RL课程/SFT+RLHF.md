...
- **技术原理**：SFT 使用的是**交叉熵损失（Cross-Entropy Loss）**。在训练时，模型每输出一个 token，系统就会拿它和标准答案（Ground Truth）对比。如果模型想输出 "我"，但标准答案是 "你"，系统就会强行把梯度的方向往 "你" 上拉。

  **在你的作业中**：你用 DeepSeek R1 生成的带有 `<think>` 标签的数据来做 SFT，其实就是在逼迫 Qwen 模型**模仿** R1 的说话语气和思维步骤。
...
| **维度**         | **SFT (监督微调)**                                           | **RL / RLHF (强化学习)**                                     |
| ---------------- | ------------------------------------------------------------ | ------------------------------------------------------------ |
| **指导方式**     | **“你必须确切地这么说”**（提供完整的高质量回答轨迹）         | **“你自己试，我只告诉你哪个好”**（只提供正确答案或偏好打分） |
| **数据获取成本** | **极高**。需要人类专家或极强的模型写出完美的、长篇的推导过程。 | **较低/极低**。只需要问题和最终答案（在数学代码领域），或者让人类做“二选一”的选择题（RLHF）。 |
| **能力天花板**   | **受限于展示的数据（Demonstration）**。模型只能逼近数据质量的上限。 | **具有探索性（Exploration），能突破上限**。模型可能会自己发现比人类/SFT数据更聪明、更高效的解题策略。 |
| **幻觉问题**     | 容易产生幻觉。如果逼迫小模型模仿它内部知识库里没有的推理，它就会“一本正经地胡说八道”。 | 缓解幻觉。RL 鼓励模型在自己现有的知识边界内寻找最优解，而不是强行模仿别人。 |
