...
python cs336_basics/valloss.py --config config/01_llama_vs_vanilla.yaml
# 2. 实验二：Norm 位置对比 (Pre-Norm vs Post-Norm)
python cs336_basics/train.py --config config/02_norm_location.yaml
python cs336_basics/valloss.py  --config config/02_norm_location.yaml
# 3. 实验三：位置编码对比 (RoPE, NoPE, Sinusoidal, Learned)
python cs336_basics/train.py --config config/03_positional_encoding.yaml
python cs336_basics/valloss.py  --config config/03_positional_encoding.yaml
# 4. 实验四：权重绑定对比 (Tied vs Untied)
python cs336_basics/train.py --config config/04_weight_tying.yaml
python cs336_basics/valloss.py --config config/04_weight_tying.yaml
# 5. 实验五：激活函数对比 (ReLU vs SwiGLU vs GELU)
python cs336_basics/train.py --config config/05_activation.yaml
python cs336_basics/valloss.py --config config/04_weight_tying.yaml
```
...
```
python cs336_basics/train.py --config config/06_llama.yaml
```

**训练一个“复古”的 2017 风格模型（Post-Norm, ReLU, Sinusodial PE）：**

```
python cs336_basics/train.py --config config/07_vanilla.yaml
```
